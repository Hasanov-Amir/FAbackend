...
def show_tovar(request, tovarslug):
    ...
    try:
        salesman = auth.get_user(request)
        fav_tovari = salesman.user_favorite.all()
        cart_tovari = salesman.user_cart.all()
    except:
        salesman = ""
        fav_tovari = []
        cart_tovari = []
    ...
    try:
        tovar_property = tovar.properties['lineyka'][1]
        from_one_lineyka = Tovar.objects.filter(properties__lineyka__1=tovar_property)[:10]
    except:
        from_one_lineyka = []

    pred_tovarov = Tovar.objects.filter(properties=tovar.properties)
    ...
    context = {
        ...
        'cart_tovari': cart_tovari,
        'comments': comments,
        'from_one_lineyka': from_one_lineyka,
        'pred_tovarov': pred_tovarov
    }
...
def find_page(request):
    srch = request.GET.get("name", None)
    r = srch.lower().split()
    ...
    titles = Tovar.objects.values_list('title_small', flat=True).distinct()
    titles = list(titles)
    contents = Tovar.objects.values_list('content_small', flat=True).distinct()
    contents = list(contents)

    alls = titles + contents
    ...
    for index, user_word in enumerate(r):
        for word in words:
            value = Levenshtein.distance(user_word, word)
            variants.append([value, word])
        if variants:
            try:
                variant = min([i[0] for i in variants if i[0] <= 3])
                variant = [i for i in variants if i[0] == variant]
                r[index] = variant[0][1]
            except:
                pass
...
def filter_of_tovar(request):
    ...
    try:
        ...
        tovari = eval(tovari)
        tovari = tovari.filter(podpodcat__name=ppc)
    except:
...
def crud_rating_comments(request):
    ...
    try:
        ...
        tovar_comments = len(tovar_rating)
        tovar_recommendation = len([i.rating for i in tovar_rating if i.rating >= 4])
        tovar_rating = round(sum([i.rating for i in tovar_rating])/len(tovar_rating), 1)
        tovar.rating = tovar_rating
        tovar.comments_count = tovar_comments
        tovar.recommendation = round((tovar_recommendation*100)/tovar_comments, 1)
        tovar.save()
    except:
        pass
